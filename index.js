...
function profilListesi(fenomenler) {
  const yeniDizi = fenomenler.map((fenomen) => fenomen.profile);
  return yeniDizi;
}
...
function fenomenSil(fenomenler, indeks) {
  /*kod*/
  const yeniDizi2 = [...fenomenler];
  yeniDizi2.splice(indeks, 1);
  return yeniDizi2;
}
...
function enFenomenler(fenomenler) {
  /*kod*/
  return fenomenler.filter((fenomen) => fenomen.followers > 100000000).map((fenomen) => fenomen.profile);
}
...
console.log(fenomenGonderimSayisi(fenomenler, "Will Smith")); // 136
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  const platformFenomenleri = fenomenler.filter((fenomen) => fenomen.platform === platform && fenomen.posts !== "NA");

  let maxPostsFenomen = platformFenomenleri[0];
  for (let i = 1; i < platformFenomenleri.length; i++) {
    if (platformFenomenleri[i].posts > maxPostsFenomen.posts) {
      maxPostsFenomen = platformFenomenleri[i];
    }
  }

  return maxPostsFenomen.profile;
}
